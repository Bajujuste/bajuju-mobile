...
export default function AdminReportsScreen() {
          ...
          collected.push(
            ...((result.data || []) as LooseRow[])
              ...
              .map((row, index) => ({
                ...
                title: firstText(row, ['title', 'titolo', 'reason', 'motivo', 'type', 'tipo'], 'Segnalazione'),
                subtitle: firstText(
                  row,
  ...
  const onArchive = useCallback(
    async (item: ReportItem) => {
      Alert.alert('Archiviare segnalazione', 'Vuoi spostare questa segnalazione in Archivate?', [
  ...
  return (
    ...
    >
      <View style={styles.headerCard}>
        ...
        <Text style={styles.text}>Segnalazioni aperte: {reports.length}</Text>
        ...
        <View style={styles.topActions}>
          ...
          <Pressable style={styles.archiveTopButton} onPress={() => router.push('/admin-reports-archived')}>
            <Text style={styles.archiveTopButtonText}>Archivate</Text>
          </Pressable>
        </View>
      </View>
      ...
      {loading ? (
        <View style={styles.card}>
          <ActivityIndicator />
          <Text style={styles.loadingText}>Carico segnalazioni...</Text>
        </View>
      ) : null}

      <View style={styles.card}>
        {reports.length === 0 ? (
          <Text style={styles.emptyText}>Nessuna segnalazione aperta.</Text>
        ) : (
          reports.map((item) => (
            <View key={`${item.table}-${item.id}`} style={styles.reportBox}>
              ...
              <Pressable style={styles.archiveButton} onPress={() => onArchive(item)}>
                <Text style={styles.archiveButtonText}>Archivia segnalazione</Text>
              </Pressable>
            </View>
          ))
        )}
